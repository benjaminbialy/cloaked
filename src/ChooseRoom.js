...
function ChooseRoom(props) {
	...
	const handleNewRoom = (e) => {
		...
		setRoomPassword((prev) => prev.trim());
		...
		if (!arrRoomNames.includes(roomName)) {
			set(newCCRef, {
				message:
					"Welcome, to invite people to the room, share the room name and password with them.",
				sender: "Admin",
			});
			...
			setError(
				"Sorry, the room name you entered already exists, try something else."
			);
			setAttemptName("");
			setAttemptPassword("");
		}
	...
	const handleRoomJoin = (e) => {
		e.preventDefault();
		setAttemptName((prev) => prev.trim());
		setAttemptPassword((prev) => prev.trim());
		...
		if (arrRoomNames.includes(attemptName)) {
			...
			if (checkPassword(attemptName, attemptPassword)) {
				...
				let indexOfRoomID = roomDataArray.findIndex(
					(id) => id.roomName == attemptName
				);
	...
	return (
		<div className="choose__room">
			...
			<div className="choose__room--forms">
				...
				<RoomForm
					purposeOfForm={"Join an existing room"}
